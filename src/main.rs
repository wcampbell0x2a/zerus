...
fn main() {
    ...
    let vendors = get_vendors(&args.vendor_path);

    download_and_save(&args.mirror_path, vendors);
}
...
fn get_vendors(vendor_path: &str) -> Vec<(String, String)> {
...
fn download_and_save(mirror_path: &Path, vendors: Vec<(String, String)>) {
